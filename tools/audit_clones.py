...
for path in sorted(Path('clones').glob('*.html')):
    ...
    print(path.name, 'hits=' + (','.join(hits) if hits else 'none'), 'credit=' + str('Abdul Anas' in text), 'html=' + str(bool(soup.html)), 'body=' + str(bool(soup.body)))
    if hits:
        raise SystemExit(f'residual branding in {path.name}: {hits}')
    if 'Abdul Anas' not in text or not soup.html or not soup.body:
